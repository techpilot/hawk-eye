"""
To read the camera - Computer Vision
alarm sound package
"""
import cv2
import winsound
...
def alarm(video_id):
    camera = cv2.VideoCapture(video_id, cv2.CAP_DSHOW)
    while camera.isOpened():
        ...
        for c in contours:
            ...
            cv2.rectangle(frame1, (x, y), (x+w, y+h), (0, 250, 0), 2)  # colour of contours and thickness
            winsound.PlaySound('alert.wav', winsound.SND_ASYNC)
        '''q is is break key like to stop the cam or like emergency wait key'''
        if cv2.waitKey(10) == ord('q'):
            break
        # cv2.imshow("Stephen's Cam", frame1)
